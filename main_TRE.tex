...
Let $F(b,s|X,\beta)=f(b,s|X,\beta)+\varepsilon_{bs}$ be the match value function of match $(b,s)$ where $\varepsilon_{bs}$ is drawn from zero median distribution. The equilibrium one-to-one matching allocation $\{m(b,s)\}_{b\in\mathcal{N}_b,s\in\mathcal{N}_s}$ is calculated by the following linear programming problem proposed by \cite{shapley1971assignment}:
\begin{align*}
