...
In counterfactual simulations, we find that the prohibition of mergers between firms in the same country affects only the merger configuration, that is, buyer-seller-match pairs.
\end{abstract} 
...
\begin{table}[!htbp]
  \begin{center}
      ...
      \input{figuretable/number_of_mergers_counterfactual}
  \end{center}\footnotesize
  %Note: 
\end{table} 
...
Since only one merger of firms in different countries happened between 1966 and 1990, we focus on the two regimes between 1991 and 2005 and between 2006 and 20022.
...
Second, as an interesting finding, only 22.2\% of the counterfactual merger pairs are the same as actual pairs.
This is because the objective function based on the subset matching pairs and modified joint production function generates a different equilibrium matching outcome from the one based on the full matching pairs and original joint production function. 
Note that the above different configurations based on an equilibrium matching which is stronger than pairwise stability matching provide just one of the potential pairwise stability matchings.
Intuitively, the prohibition of mergers between firms in the same country, all firms XXX.
Therefore, we find that the prohibition of mergers between firms in the same country affects only the merger configuration, that is, buyer-seller-match pairs.
